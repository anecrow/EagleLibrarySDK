...
export function ConsoleLog(msg: string, opt?: Option) {
  opt ??= { flag: "INFO" };
  opt.color ??= "#fff";
  opt.title ??= title;
  console.log(`${msg}\n%s`, ...icon(opt));
}
export function ConsoleWarn(msg: string, opt?: Option) {
  opt ??= { flag: "WARN" };
  opt.color ??= "#ff0";
  opt.title ??= title;
  console.warn(`${msg}\n%s`, ...icon(opt));
}
export function ConsoleError(msg: string, opt?: Option) {
  opt ??= { flag: "INFO" };
  opt.color ??= "#f66";
  opt.title ??= title;
  console.error(`${msg}\n%s`, ...icon(opt));
